...
4. Before writing any ticket/changelog changes (step 5), make sure the working tree can actually update `CHANGELOG.md`: if the current branch isn't the default branch, stash any uncommitted work — `git stash push --include-untracked` (plain `git stash` skips untracked files, which can block the upcoming checkout if paths conflict) — check out the default branch, and pull. If anything is already staged before this step runs (leftover from something unrelated), don't fold it into step 5's commit — `git status --short` first and stop to ask if the tree isn't clean beyond what this step itself is about to stash.
...
7. If step 4 switched off the original branch, return to it now (`git checkout -`, then `git stash pop` if something was stashed) — unconditionally, whether step 6 succeeded or failed.
