...
# Built from raw UTF-8 byte escapes (\xHH), not \U code-point escapes --
# \U depends on the *calling* shell's locale being multibyte-aware to
# encode the character at all and silently produces nothing under a C/
# POSIX locale (exactly what this repo's own sandboxed CI container has),
# which was quietly turning this fixture into plain text with no tag
# characters in it at all -- a false BYPASS in CI, not a real one. \xHH
# byte escapes emit the literal bytes regardless of locale, matching how
# these characters actually arrive in real fetched content: as bytes.
TAG_CHARS=$(printf '\xf3\xa0\x81\xa8\xf3\xa0\x81\xa9')
ZW_CLUSTER=$(printf '​​​​​')
ZW_SINGLE=$(printf '‍')
