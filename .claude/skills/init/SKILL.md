...
3. **Create the scaffold**, skipping anything the user chose to keep. Every copy sources from `$ASSET_ROOT` (step 1) regardless of what may or may not already exist at the project-local destination:
   - Copy `$ASSET_ROOT/templates/` → `./templates/` (all seven template files).
