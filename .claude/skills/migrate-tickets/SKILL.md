...
- Add to the project board and set Ticket Status to whatever the source folder implies, using `gh project item-add`/`item-edit` with ids resolved from `.claude/github-project-config.json` — identical mechanics to `/bg:wrap-up`'s status-setting step; don't reinvent that pattern here.
- Record `<old-id> → <new #N>` in an in-memory mapping table.

**Per question** (skip per Step 5 if already migrated): create with the `type:question` label, the marker line, and body reformatted to `templates/question.md`'s structure. Not added to the project board — questions aren't board items (see `CLAUDE.md`'s "Questions" section). If it came from `questions/answered/`: create it open, then immediately close it with the original `## Answer`/`## Resolution Impact` content as the closing comment (GitHub has no create-already-closed call, so this is always two steps). If from `questions/open/`: leave it open. Record `<old-id> → <new #N>` in the same mapping table.

## Step 7 — Pass 2: remap cross-references

Every issue just created may still reference old ids inside its `## Dependencies` (tickets) or `## Blocks` (questions) section, since Step 6 copied body content verbatim before every id had a number. For each created issue, re-read its body, replace every old-id token (`feat-N`, `rem-N`, `q-N`) found there with `#<mapped number>` from Step 6's table, and write the body back (`issue_write`/`gh issue edit --body`). If a token doesn't resolve (it referenced something outside this migration's scope — already done and archived elsewhere, or simply not found), leave it as literal text and flag it in the Step 8 report rather than guessing or silently dropping it.
